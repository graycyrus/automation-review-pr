...
**CRITICAL**: You MUST post the review using `gh api --input -` with a heredoc as shown below. Do NOT write the review body to a temp file and use `@/path/to/file` — that posts the literal string `@/path/to/file` as the review body. Do NOT use `-f body=@file`. Always inline the JSON via heredoc stdin.
